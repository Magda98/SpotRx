...
import { SearchResponse, Track, TracksResponse } from '@app/shared/interfaces/track';
...
export class MockBackendInterceptor implements HttpInterceptor {
	...
	> {
		...
		if (req.url.endsWith('playlists/50Gsv3p7qLLPVzfPBu8UcO') && req.method == 'GET') {
		...
		if (req.url.endsWith('browse/featured-playlists') && req.method == 'GET') {
			return new Observable<HttpResponse<FeaturedPlaylistResponse>>((observer) => {
				observer.next(
			...
			return new Observable<HttpResponse<TracksResponse<Track>>>((observer) => {
				observer.next(
					new HttpResponse<TracksResponse<Track>>({
						body: {
							...savedTracks,
							items: savedTracks.items.map((item: TracksResponse['items'][number]) => ({
								...item.track,
							})),
						},
						status: 200,
